...
DOCTEST_TEST_CASE("put::ip4_addr_port::from_network_order(in_addr)")
{
    ...
    // WHEN
    const auto addr_port = put::ip4_addr_port::from_network_order(in_addr, port);
...
{
    // GIVEN
    constexpr put::ip4_addr::integer_type addr = ben::big_to_native(0x0A000001u);
...
DOCTEST_TEST_CASE("put::ip4_addr_port::to_sockaddr_in")
{
    ...
    // WHEN
    const auto sockaddr = addr_port.to_sockaddr_in();

    // THEN
    DOCTEST_CHECK_EQ(sockaddr.sin_family, AF_INET);
    DOCTEST_CHECK_EQ(ben::big_to_native(sockaddr.sin_port), port);
    DOCTEST_CHECK_EQ(sockaddr.sin_addr.s_addr,
                     addr_port.addr_.to_in_addr().s_addr);
    DOCTEST_CHECK_EQ(
        ::memcmp(sockaddr.sin_zero, "\0\0\0\0\0\0\0\0", sizeof(sockaddr.sin_zero)),
        0);
}
...
DOCTEST_TEST_CASE("put::ip6_addr_port::from_network_order(in6_addr)")
{
    ...
    // WHEN
    const auto addr_port = put::ip6_addr_port::from_network_order(in_addr, port);
...
DOCTEST_TEST_CASE("put::ip6_addr_port::to_sockaddr_in")
{
    ...
    // THEN
    DOCTEST_CHECK_EQ(sockaddr.sin6_family, AF_INET6);
    DOCTEST_CHECK_EQ(ben::big_to_native(sockaddr.sin6_port), port);
    DOCTEST_CHECK_EQ(sockaddr.sin6_flowinfo, 0u);
    DOCTEST_CHECK_EQ(::memcmp(&sockaddr.sin6_addr, &in_addr,
                              sizeof(sockaddr.sin6_addr)),
                     0);
    DOCTEST_CHECK_EQ(sockaddr.sin6_scope_id, 0u);
}
